...
| 직원 관리 | 전체 관리자·지점 관리자: 지점 직원 카드(원장·실장·디자이너·인턴·데스크, 담당 시술, 정기 휴무, 입사일·근속, 시술/제품 인센티브율, 미용사 면허번호, 보건증 만료일, 재직·휴직·퇴사). 요일별 근무 인원·디자이너 수와 휴무자를 보여 주는 **주간 근무 현황**(디자이너가 한 명도 없는 요일은 빨갛게 표시), 보건증 만료·30일 안 만료·미등록 알림. 앱 계정이 없는 직원(인턴 등)도 등록합니다 |
...
cd salon
npm install
npm test   # DB 139 passed · admin-users 23 passed
```
