...
| 사용자 관리 | 사용자 초대·역할 변경·사용 중지, 대기 중인 초대 관리 |
...
| 사용자 | 전 지점 사용자 초대·배정·이동·역할 변경(전체 관리자 포함)·사용 중지 | 자기 지점 사용자 초대·역할 변경(직원↔지점 관리자)·사용 중지·지점에서 제외 | — |
| 품목 | 공용 품목 목록 추가·수정 | 자기 지점 안전재고·보관 위치 | — |
| 재고 | 전 지점 (상단에서 지점 선택) | 자기 지점, 재고 실사, 모든 기록 취소 | 자기 지점 입고·사용·판매·폐기, 본인 기록 10분 안 취소 |

**계정은 초대로 만듭니다.**

1. 관리자가 **사용자 관리 → 사용자 초대**에서 이메일, 역할, 지점을 정합니다.
2. 초대받은 사람이 로그인 화면의 **초대받은 이메일로 가입하기**로 가입합니다.
3. 가입하는 순간 지점과 역할이 자동으로 적용됩니다. 초대는 14일 뒤 만료됩니다.

초대 없이 가입한 계정은 지점이 없어서 아무 데이터도 볼 수 없습니다. 전체 관리자는 **사용자 관리 → 지점: 미배정**에서 이런 계정을 찾아 배정할 수 있습니다.

**잠김 방지 규칙**: 누구도 자기 자신의 역할·지점·사용 여부를 바꿀 수 없고, 마지막 전체 관리자는 강등하거나 중지할 수 없습니다. 사용 중지된 계정은 로그인해도 아무것도 볼 수 없습니다.

모든 규칙은 화면이 아니라 **DB 함수와 RLS**에서 검사합니다. 화면을 조작해도 권한 밖의 작업은 거부됩니다.

## 데이터 설계

```
branches ─┬─ profiles (사용자 ↔ 지점·역할·사용 여부)
          ├─ invitations (대기 중인 초대)
...
- 앱은 테이블을 직접 수정할 수 없습니다. 모든 변경은 역할을 검사하는 DB 함수(`record_movement`, `revert_movement`, `save_product`, `set_branch_item`, `save_branch`, `invite_user`, `update_user`, `cancel_invitation`, `list_users`)로만 합니다. 그래서 재고 변동은 **모두 기록**되고 재고가 0 밑으로 내려가지 않습니다.
...
4. **Authentication → Sign In / Providers → Email**에서 다음 두 가지를 확인합니다.
   - **Allow new users to sign up**: 켬. 초대받은 사람이 직접 가입합니다.
   - **Confirm email**: 켬. 초대받은 이메일의 주인만 가입을 마칠 수 있습니다.
5. **Authentication → URL Configuration**의 **Site URL**에 배포 주소(예: `https://hplace-salon.onrender.com`)를 넣습니다. 인증 메일의 링크가 이 주소로 돌아옵니다.

> Supabase 무료 플랜의 기본 메일 발송은 시간당 몇 통으로 제한됩니다. 직원이 많아지면 **Authentication → Emails → SMTP Settings**에 회사 메일 서버를 연결하세요.
...
- 발주서 작성·거래처 관리
- 유통기한·로트 관리 (염모제, 펌제)
- 초대 메일을 앱에서 바로 보내기 (지금은 가입 주소를 복사해 전달)
