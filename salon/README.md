...
| 제품 관리 | 모든 사용자: **이 지점 사용** 켜기/끄기(끄면 그 지점의 재고 목록·입출고 등록에서만 숨김, 다른 지점은 그대로) · 전체 관리자: 제품 등록·수정, 전 지점 사용 중지 · 지점 관리자: **새 제품 등록**(모든 지점에 재고 0으로 준비, 품목 코드는 카테고리별로 자동 부여: 클리닉 → CN-001, 소모품 → SP-001 …), 자기 지점 안전재고·보관 위치, 제품 이름·브랜드·카테고리·고객 판매용 수정(모든 지점에 반영), **단위·매입가·판매가는 자기 지점에만** 적용(목록에 "지점 가격"과 공통 값 표시, "공통 값으로 되돌리기"). 전체 관리자는 수정할 때 "이 지점만 / 모든 지점 같은 값"을 고릅니다. 입출고의 매입가·판매가 기록과 재고 금액도 지점 가격을 씁니다. 수정 창의 "사용 중"은 지점 관리자에게는 이 지점 사용 여부로 동작합니다. 품목 코드와 모든 지점 사용 중지는 전체 관리자만 |
