...
| 직원 관리 | 전체 관리자·지점 관리자: 지점 직원 카드(**직원 사진**, 원장·실장·디자이너·인턴·데스크, 담당 시술, 정기 휴무, 입사일·근속, 시술/제품 인센티브율, 미용사 면허번호, 보건증 만료일, 재직·휴직·퇴사). 요일별 근무 인원·디자이너 수와 휴무자를 보여 주는 **주간 근무 현황**(디자이너가 한 명도 없는 요일은 빨갛게 표시), 보건증 만료·30일 안 만료·미등록 알림. 앱 계정이 없는 직원(인턴 등)도 등록합니다 |
| 근무표 | 모든 사용자(보기) · 지점 관리자 이상(수정): 월별 근무표. 정기 휴무 위에 날짜별 연차·반차·병가·교육·추가 휴무·대체 근무를 기록하고, 날마다 근무 인원과 디자이너 수를 보여 줍니다(디자이너 0명인 날 표시). 관리자에게는 **연차 현황**(근로기준법 입사일 기준 발생 추정, 사용·남은 일수)도 보입니다 |
...
cd salon
npm install
npm test   # DB 177 passed · admin-users 23 passed
```
