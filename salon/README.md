...
| 제품 관리 | 모든 사용자: **이 지점 사용** 켜기/끄기(끄면 그 지점의 재고 목록·입출고 등록에서만 숨김, 다른 지점은 그대로) · 전체 관리자: 제품 등록·수정, 전 지점 사용 중지 · 지점 관리자: **새 제품 등록**(모든 지점에 재고 0으로 준비, 품목 코드는 카테고리별로 자동 부여: 클리닉 → CN-001, 소모품 → SP-001 …), 자기 지점 안전재고·보관 위치, 제품 이름·가격(매입가·판매가)·고객 판매용 수정(모든 지점에 반영). 코드·브랜드·카테고리·단위는 전체 관리자만 |
...
| 제품 | 제품 등록·수정, 카테고리 관리, 전 지점 사용 중지 | 새 제품 등록, 제품 이름·가격·고객 판매용 수정, 자기 지점 안전재고·보관 위치·사용 여부 | 자기 지점 사용 여부 |
...
cd salon
npm install
npm test   # DB 197 passed · admin-users 23 passed
```
