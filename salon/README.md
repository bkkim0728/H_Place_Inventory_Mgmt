...
| 제품 관리 | 전체 관리자: 제품 등록·수정 · 지점 관리자: **새 제품 등록**(모든 지점에 재고 0으로 준비, 품목 코드는 카테고리별로 자동 부여: 클리닉 → CN-001, 소모품 → SP-001 …), 자기 지점 안전재고·보관 위치. 이미 등록된 제품의 이름·코드·가격 변경은 전체 관리자만 |
...
| 제품 | 제품 등록·수정, 카테고리 관리 | 새 제품 등록, 자기 지점 안전재고·보관 위치 | — |
...
cd salon
npm install
npm test   # DB 188 passed · admin-users 23 passed
```
