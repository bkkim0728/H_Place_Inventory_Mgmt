    ...
    return {
      ...
      saveProduct: (branchId, p) =>
        run(sb.rpc('save_product', {
          ...
          p_price_scope: p.priceScope || null,
        })),
  ...
  function demoApi() {
    ...
    return {
      ...
      async recordMovement({ branchId, productId, type, quantity, memo, staffId }) {
        ...
        row.stock = after;
        row.updated_at = new Date().toISOString();
        const v = priced(row, p);
        const m = insertMovement({
          branch_id: branchId, product_id: productId, type, quantity: delta, stock_after: after, unit_cost: v.cost_price,
          unit_price: type === 'sale' ? v.retail_price : null, staff_id: staffId || null, memo: trimOrNull(memo),
        });
        save();
