...
ok(mp.name === '이름 변경' && mp.brand === '딴브랜드' && mp.category === '도구' && mp.is_retail === true, 'manager changes name, brand, category and 고객 판매용');
...
const v1 = await as(mgr, () => one(`select unit, cost_price, retail_price, own_prices, base_cost_price from inventory_view where branch_id=$1 and product_id=$2`, [b1, mgrPid]));
...
const v2 = await as(admin, () => one(`select unit, cost_price, retail_price, own_prices from inventory_view where branch_id=$1 and product_id=$2`, [b2, mgrPid]));
ok(v2.unit === '개' && v2.cost_price === 100 && v2.retail_price === null && v2.own_prices === false, 'another branch keeps the shared values');
...
ok((await as(mgr, () => one(`select unit from movement_view where product_id=$1 limit 1`, [mgrPid]))).unit === '통', 'movement history shows the branch unit');
await as(mgr, () => q(`select save_product($1,$2,'MG-1','이름 변경','딴브랜드','도구','개',100,null,true,2,null,true,'all')`, [b1, mgrPid]));
